...
def edit_profile(request):
    if request.method == 'POST':
        form = ProfileForm(request.POST, instance=request.user.userprofile)  # Pass the userprofile instance

        if form.is_valid():
            form.save(user=request.User)  # Pass the current user instance
            return redirect('login:view_profile')

    else:
        form = ProfileForm(instance=request.user.userprofile)  # Pass the userprofile instance
        args = {'form': form}
